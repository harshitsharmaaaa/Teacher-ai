...
import { streamChat } from "@/lib/sream-chat";
...
export async function POST(req: NextRequest) {
    ...
    if(eventType==="call.session_started"){
        ...
        if(!userId || !channelId || !message){
            return NextResponse.json({error:"missing userId, channelId or message"},{status:400});
        }
        ...
        if(userId !==existingAgent.id){
            ...
            const gptResponsetext = await gptResponse.choices[0].message.content; 
            if(!gptResponsetext){
                return NextResponse.json({error:"GPT response is empty"},{status:400});
            }

            const avatarUrl = await generateAvatarUri({
